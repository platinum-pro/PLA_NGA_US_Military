...
calculate_p50 <- function(alpha, Qo, k_fixed, data_subset, target_q = 50) {
  ...
  lower_bound <- min(data_subset$Odds_Against[data_subset$Odds_Against > 0], na.rm = TRUE)
  upper_bound <- max(data_subset$Odds_Against, na.rm = TRUE)
...
fit_demographic_group <- function(data_subset, group_label, k_fixed) {
  valid_data <- data_subset[!is.na(data_subset$value) & data_subset$Odds_Against > 0, ]
  ...
  fit <- tryCatch({
    nls(
      formula = value ~ Koff(Odds_Against, alpha, Qo, k_fixed),
      data    = valid_data,
  ...
  if (!is.null(fit)) {
    ...
    if (!is.na(p50)) {
      ...
      cat("P50 (as likelihood of success)    =", round(p50_prob, 2), "%\n")
    } else {
...
create_comparison_plot <- function(data, demographic_pair, fit_results_1, fit_results_0,
  ...
  data_1 <- data.frame(Odds_Against = data$Odds_Against, value = data[[col_1]]) %>%
    filter(!is.na(value) & Odds_Against > 0)
  data_0 <- data.frame(Odds_Against = data$Odds_Against, value = data[[col_0]]) %>%
    filter(!is.na(value) & Odds_Against > 0)
  
  odds_range <- 10^seq(log10(min(data$Odds_Against[data$Odds_Against > 0])),
                       log10(max(data$Odds_Against)), length.out = 100)
  plot_data <- data.frame(Odds_Against = odds_range)
  
  # Overall curve
  if (!is.null(overall_data$fit)) {
    params_overall <- coef(overall_data$fit)
    plot_data$overall_pred <- Koff(
      odds_range,
  ...
  if (!is.null(overall_data$fit)) {
    p <- p +
      geom_line(
        data  = plot_data,
        aes(x = Odds_Against, y = overall_pred),
        color    = "black",
        linewidth = 0.6,
        alpha    = 1,
        linetype = "dashed"
  ...
  if (nrow(data_1) > 0) {
    p <- p + geom_point(
      data  = data_1,
      aes(x = Odds_Against, y = value, color = demographic_pair$label1),
      shape = 16,
      alpha = 0.6,
      size  = 1.5
    )
  ...
  if (nrow(data_0) > 0) {
    p <- p + geom_point(
      data  = data_0,
      aes(x = Odds_Against, y = value, color = demographic_pair$label0),
      shape = 17,
      alpha = 0.6,
      size  = 1.5
    )
  ...
  if (!is.null(fit_results_1$fit)) {
    p <- p + geom_line(
      data  = plot_data,
      aes(x = Odds_Against, y = group1_pred, color = demographic_pair$label1),
      linewidth = 0.8
    )
  }
  ...
  if (!is.null(fit_results_0$fit)) {
    p <- p + geom_line(
      data  = plot_data,
      aes(x = Odds_Against, y = group0_pred, color = demographic_pair$label0),
      linewidth = 0.8
    )
  }
...
valid_data_all <- data[!is.na(data$All) & data$Odds_Against > 0, ]

fit_all <- tryCatch({
  nls(
    formula = All ~ Koff(Odds_Against, alpha, Qo, k_global),
    data    = valid_data_all,
    start   = list(alpha = 0.0000001, Qo = 100),
    algorithm = "port",
    lower   = c(alpha = 0, Qo = 0),
