...
# Calculate Likelihood from Odds_Against if not already present
if (!"Likelihood" %in% names(df_long)) {
  df_long$Likelihood <- 100 / (1 + df_long$Odds_Against)
}
...
plot_with_line <- function(subject_data, subject_code) {
  ...
  ggplot(subject_data, aes(x = Likelihood, y = Willingness)) +
